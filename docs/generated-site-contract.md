...
- Element and section `settings.animation.effect` may use the animation options advertised by the editor. A self-hosted Motion runtime enhances those effects when needed; the existing CSS fallback remains, duration and delay stay bounded, and animation is disabled when a visitor prefers reduced motion.
...
- Use `three-scene` for an asset-free procedural Three.js visual. Choose `display.preset` from `product-stage`, `crystal`, or `wave`; set `tone`, `accent`, `motion`, `interactive`, `ratio`, `camera`, `lighting`, and `finish` only to values advertised by the registry editor.
