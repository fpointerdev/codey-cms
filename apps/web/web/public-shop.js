import { elements, escapeHtml, hasPermission, moduleEnabled, setStatus, state, translateString } from "./core.js";
...
function renderShopAdminLink(href, label) {
  if (!state.user || !moduleEnabled("products") || !hasPermission("update", "products")) return "";

  return `<a class="visual-editor-entry shop-admin-entry" href="${escapeHtml(href)}" data-dashboard-link><span aria-hidden="true">&#9998;</span> ${escapeHtml(label)}</a>`;
}
...
export function renderShopListing({ products = [], categories = [], attributes = [], route = {}, settings = {}, pagination = {} }) {
  ...
  elements.page.innerHTML = `
    ${renderShopAdminLink(adminHref("shop-configuration"), "Edit shop design")}
...
export function renderProductDetail(product, settings = {}) {
  ...
  elements.brand.href = "/";
  elements.page.innerHTML = `
    ${renderShopAdminLink(adminHref("product-editor", product.slug), "Edit product")}
    ${renderProductDetailContent(product, renderOptions(settings))}
  `;
  elements.footer.innerHTML = "";
  document.body.classList.remove("auth-enabled", "dashboard-enabled", "editor-enabled");
